...
public class XMLpathgetincidents	 {
	
    @Test
	public void XMLformatgetincidents() {
...
//Step 3//Request type/GET  // wecan also gine lieke RestAssured.get("incident")
		 Response response = RestAssured
				 ...
				 .accept(ContentType.XML)  //server will gib=ve response as we expect
				 .get(); 
...
//Step 6: print the response 
		...
		//result.number
		//For XML path respinse use XML path methiod instean do json opath
		XmlPath xmlPathresponse = response.xmlPath();
		
		List<String> list = xmlPathresponse.getList("response.result.sys_id");  //if very sure abt data type change list type as string
